...
struct SplitRangeCSRMatrix{T, Ti, V, Vi1, Vi2} <: AbstractMatrix{T}
    contiguous_ptr::Vi1
    contiguous_vals::V
    scattered_ptr::Vi1
    scattered_inds::Vi2
...
SplitRangeCSRMatrix(cp::Vi1, cv::V, sp::Vi1, si::Vi2, sv::V) where {V, Vi1, Vi2} =
    SplitRangeCSRMatrix{eltype(V), eltype(Vi1), V, Vi1, Vi2}(cp, cv, sp, si, sv)
